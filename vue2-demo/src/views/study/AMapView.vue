...
export default {
  ...
  mounted() {
    ...
    AMapLoader.load({
      ...
      plugins: ['AMap.Geocoder', 'AMap.Marker', 'AMap.Geolocation', 'AMap.ToolBar', 'AMap.Scale', 'AMap.MapType'], // 需要使用的的插件列表，如比例尺'AMap.Scale'等
    })
      .then((AMap) => {
          ...
          if (status == 'complete') {
            ...
            geocoder.getAddress([result.position.lng, result.position.lat], function (geostatus, georesult) {
              if (geostatus === 'complete' && georesult.info === 'OK') {
                ...
                tools.ajax('/api/amap/weatherInfo', { city: adcode }, (data) => {
                  logger.debug('天气信息', data)
                })
              } else {
                logger.debug('没有获取到逆地理编码', geostatus, georesult)
              }
            })
          } else {
            // onError(result)
